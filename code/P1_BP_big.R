...
for(ii in 1:numNodes){
  ...
  # set pi values for non-roots = all ones
...
while( notConverged && iter <= MAX.ITER){
  ...
  for(ii in 1:numNodes){
    ...
    #For every value that x can take on, get incoming messages
    for(xx in 1:currNode$scopeLength){
      x.i <- currNode$scope[xx]
    ...
    } #End of calculating lambda and pi vals over all values of x
  ...
  }#End of looping over every node, we have all messages.
...
#Check beleifs
for(ii in 1:numNodes){
  print(bpList[[iter]][[ii]]$node)
  print(bpList[[iter]][[ii]]$bel)
  print("-")
